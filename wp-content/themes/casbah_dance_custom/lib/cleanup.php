...
class Roots_Nav_Walker extends Walker_Nav_Menu {
	...
	function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
		...
		error_log(print_r($item->title, true));
		$test_match = 'test match:' . preg_match($find_nav_titles, $item->title);
		error_log(print_r($test_match, true));
		$test_replace = 'test replace:' . preg_replace($find_nav_titles, '', $item->title);
		error_log(print_r($test_replace, true));


		if ( preg_match( $find_nav_titles, $item->title ) > 0 ) {
			$item->title = preg_replace( $find_nav_titles, '', $item->title );
			$item_output = '<li class="nav-header">' . $item->title . '</li>';
		}
